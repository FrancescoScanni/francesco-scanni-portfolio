<?php
    ...
    // CSS class mapping for caption shapes
    $shape_classes = [
        "bubble"   => "bg-blue-50 text-blue-800 p-3 text-xs font-mono rounded-2xl rounded-tl-none border border-blue-100 transition-all group-data-[active=true]/item:bg-blue-600 group-data-[active=true]/item:text-white",
        "ribbon"   => "bg-amber-50 text-amber-900 p-3 text-xs font-mono border-l-4 border-amber-500 shadow-sm transition-all group-data-[active=true]/item:bg-amber-500 group-data-[active=true]/item:text-white",
        "pill"     => "bg-emerald-50 text-emerald-800 py-2 px-5 text-xs font-mono rounded-full border border-emerald-200 text-center shadow-sm transition-all group-data-[active=true]/item:bg-emerald-600 group-data-[active=true]/item:text-white group-data-[active=true]/item:border-emerald-600",
        "skewed"   => "bg-purple-50 text-purple-800 p-3 text-xs font-mono rounded-md -skew-x-6 border-r-2 border-purple-400 text-center transition-all group-data-[active=true]/item:bg-purple-600 group-data-[active=true]/item:text-white"
    ];
?>
...
<head>
    ...
    <title>Francesco | Project Portfolio</title>
...
<body class="bg-[#F8F9FA] antialiased font-sans text-gray-900">
            ...
            <?php    
                ...
                if ($result && $result->num_rows > 0) {
                    while ($row = $result->fetch_assoc()) {
                        ...
                        $current_shape = $shape_classes[$shape_type] ?? $shape_classes['bubble'];
                        $caption_position_class = ($shape_type === 'pill') ? 'flex justify-center' : '';
                    ...
                    echo "<p class='text-slate-400 font-mono text-xs w-full text-center'>No events found in the database.</p>";
                }
            ?>
    ...
    document.addEventListener('DOMContentLoaded', () => {
        ...
        // Handle active state via IntersectionObserver
